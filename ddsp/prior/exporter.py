...
def export_latents(
    ...
    """
    Export DDSP encoder latents and control features to HDF5.

    - Writes concatenated `controls`: [num_sequences, seq_len, feature_size + latent_size]
    - Also writes separate `latents` for convenience.
    ...
    try:
        if hasattr(model, 'encoder') and hasattr(model.encoder, 'streaming'):
    ...
    for i_chunk in range((audio.size(0) + l_chunk - 1) // l_chunk):
        ...
        if hasattr(model.encoder, 'reparametrize'):
            ...
            z = mu
        z = model._smooth_latents(z).squeeze(0)  # [Tz, Dz]

        # Downsample features to latent frames
    ...
    with h5py.File(out_path, "w") as h:
        ...
        # Also provide separate views for flexibility
